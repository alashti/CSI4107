...
public class InvertedIndex implements Serializable {
	...
	/**
	 * This object associates a tag, xml file, and index word together.
	 * Implemented by Carol-Ann Renaud
	 */
	private Parser parser = new Parser("en");

  /**
	 * 
	 */
private static final long serialVersionUID = -7332993265681647925L;

private static final int MAX_RETRIEVALS = 20;
...
/**
  ...
  /**
   * Added VB, file that results stored in
   */
  ...
  public InvertedIndex(File dirFile, short docType, boolean stem, boolean feedback) {
   ...
   indexDocuments();
    ...
    /*Add weights to tokens with tag information.  
     * Use a Parser object to traverse dom files and get a list of 
     * this information
     * 
     * Implemented by Carol-Ann Renaud
   */
    ...
    try {
    	System.out.println("Parsing DOM files");
		parser.paseFile();
  ...
  protected void indexDocuments() {
    ...
    while (docIter.hasMoreDocuments()&&docIter.nextDocument()!=null) {
      FileDocument doc = docIter.nextDocument();
      // Create a document vector for this document
     
      /////////////////////////////////////////////////////////////////////////////////
      ////////////////////////////////////////////////////////////////////////////////
      System.out.println(doc.file.getAbsolutePath()+"\r");
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#indexDocuments(java.util.List)
 */
  ...
  protected void indexDocument(FileDocument doc, HashMapVector vector) {
    ...
    for (Map.Entry<String, Weight> entry : vector.entrySet()) {
      ...
      //////////////////////////////////////////////////////////////
      int weight = parser.getWeight(token, docRef.file.getPath());
      //////////////////////////////////////////////////////////////
      
      
      // The count for the token is in the value of the Weight
      int count = (int) entry.getValue().getValue() * weight;
...
public void clear() {
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#retrieve(java.lang.String)
 */
...
public Retrieval[] retrieve(String input) {
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#retrieve(ir.vsr.Document)
 */
...
public Retrieval[] retrieve(Document doc) {
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#retrieve(ir.vsr.HashMapVector)
 */
...
public Retrieval[] retrieve(HashMapVector vector) {
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#incorporateToken(java.lang.String, double, java.util.Map)
 */
...
public double incorporateToken(String token, double count,
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#processQueries()
 */
...
/*
 * Added by Victor Buchanan to process queries from file automatically
 */

public void processQueries(File f){
	//File results = new File("Util\\Results.txt");
	BufferedWriter output;
	QueryRunner qRun = new QueryRunner(f);
	LinkedList<String> queries = qRun.getQueryList();
	String s = "";
	try {
		output = new BufferedWriter(new FileWriter("Util\\Results.txt"));
		while(!queries.isEmpty()){
			String query = queries.removeFirst();
			   HashMapVector queryVector = (new TextStringDocument(query, stem)).hashMapVector();
			      Retrieval[] retrievals = retrieve(queryVector);
			      s = s + saveToFile(retrievals,results,query);
		//	      output.append("\r");
		}
...
/*
 * Added by Victor Buchanan and Carol-Ann Renaud to create a string which represents the rults for a give query.
 */
public String saveToFile(Retrieval[] results, File r, String query){

		String s = "";
		String temp = "";
		int count;
		for (int i = 0; i < Math.min(results.length, MAX_RETRIEVALS); i++) {
			temp = results[i].docRef.file.getPath();
			temp = temp.substring(0, temp.length() - 4);
				count = i+ 1;
				s = s + query.substring(0, 10) + " 1 " + temp+" "+count+ " " + results[i].score + " CSI4107-Run \n";
		}
		return s;
}
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#presentRetrievals(ir.vsr.HashMapVector, ir.vsr.Retrieval[])
 */
...
public void presentRetrievals(HashMapVector queryVector, Retrieval[] retrievals) {
    if (showRetrievals(retrievals)) {
      ...
      do {
        ...
        if (command.equals("r") && feedback) {
          ...
          fdback.storeFeedback();  //added By Victor Buchanan store feedback to file
        ...
        if (showNumber > 0 && showNumber <= retrievals.length) {
          ...
          Browser.display(retrievals[showNumber - 1].docRef.file.getAbsolutePath());
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#showRetrievals(ir.vsr.Retrieval[])
 */
...
public boolean showRetrievals(Retrieval[] retrievals) {
  ...
  /* (non-Javadoc)
 * @see ir.vsr.InvertedInd#printRetrievals(ir.vsr.Retrieval[], int)
 */
...
public void printRetrievals(Retrieval[] retrievals, int start) {
    ...
    for (int i = start; i < Math.min(retrievals.length, start + MAX_RETRIEVALS); i++) {
      System.out.println(MoreString.padTo((i + 1) + ". ", 4) +
          ...
          " Score: " +
          MoreMath.roundTo(retrievals[i].score, 5) + "\r" +
          retrievals[i].docRef.file.getAbsolutePath() );
    }
  }
